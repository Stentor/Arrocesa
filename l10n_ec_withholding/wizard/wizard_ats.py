...
class WizardAts(models.TransientModel):
    ...
    def process_lines(self, lines):
        ...
        temp = {}
        for line in lines:
            if line.group_id in ['ret_ir', 'no_ret_ir']:
    ...
    def read_compras(self, start, end):
        ...
        dmn_purchase = [
            ('state', 'in', ['open', 'paid']),
            ('date_invoice', '>=', '{0:%Y-%m-%d}'.format(start)),
            ('date_invoice', '<=',  '{0:%Y-%m-%d}'.format(end)),
            ...
            if not inv.partner_id.type_identifier == 'pasaporte':
                ...
                detallecompras.update({
                    ...
                    'pagoExterior': {
                        ...
                        'pagExtSujRetNorLeg': self.si_no(inv.partner_id.pag_ext_suj_ret_nor_leg),
