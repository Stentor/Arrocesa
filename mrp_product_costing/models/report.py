...
class MrpCostStructure(models.AbstractModel):
    ...
    def get_lines(self, productions):
        ...
        for product in productions.mapped('product_id'):
            ...
            scraps = StockMove.search([('production_id', 'in', mos.ids), ('scrapped', '=', True), ('state', 'in', ['done', 'cost'])])
...
class ProductTemplateCostStructure(models.AbstractModel):
    ...
    @api.model
    def _get_report_values(self, docids, data=None):
        productions = self.env['mrp.production'].search([('product_id', 'in', docids), ('state', in', ['done', 'cost'])])
